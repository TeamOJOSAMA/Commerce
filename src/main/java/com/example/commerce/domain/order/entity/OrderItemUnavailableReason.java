...
public enum OrderItemUnavailableReason {
    ...
    // 상품 상태는 ON_EVENT인데 ACTIVE이면서 기간 안에 있는 이벤트가 없다. 기간이 끝났거나 아직 시작 전이다.
    // 상품당 진행 중 이벤트는 이벤트 도메인이 하나로 보장하므로 "여러 개"라는 사유는 두지 않는다.
    EVENT_NOT_IN_PROGRESS("이벤트 상품 주문은 진행 중인 이벤트 기간에만 가능합니다."),
